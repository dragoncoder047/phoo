...
module.macros.add('$', function string() {
    ...
    var str, formatCode;
    var dummy = '\x02:lesSGoDx)\x03' + Math.random();
    code = code.replaceAll(delim + delim, dummy);
    [str, code] = code.split(delim, 2);
    str = str.replace(dummy, delim + delim);
    code = code.replace(dummy, delim + delim);

    [, formatCode, code] = /^(\S*)([\s\S]*)/.exec(code);
...
module.words.add(']do-import[', async function doImport() {
    this.expect('boolean', /string|symbol/);
    var starred = this.pop();
    var name = this.pop();
    var url;
    if (type(name) === 'string') url = name;
    else url = undefined;
    var module = await this.phoo.import(name, this.module, url);
    if (starred) {
        if (this.starModules.indexOf(module) === -1) this.starModules.push(module);
    } else {
        if (this.modules.indexOf(module) === -1) this.modules.push(module);
    }
});
