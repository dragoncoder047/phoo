...
a(module, nn(1), 'lambertW', lambertw);
a(module, nn(1), 'erf', erf);
a(module, nn(1), 'erfc', erfc);
a(module, nn(2), 'besselJ', besselj);
a(module, nn(1), 'minkowski?', minkowski);
...
/* >>
word> gamma
description> This is the [gamma function](https://en.wikipedia.org/wiki/Gamma_function)
...
For a true factorial function, use this:
...
/* >>
word> lambertW
description> [Lambert *W* function](https://en.wikipedia.org/wiki/Lambert_W_function).
sed> n -- W(n)
*/
function lambertw(x) {
...
/* >>
word> erf
description> the [Gauss error function](https://en.wikipedia.org/wiki/Error_function).
sed> x -- erf(x)
*/
function erf(x) {
    var a = exp(-x * x);
...
/* >>
word> erfc
description> the complimentary error function, equal to 1-erf(x)
sed> x -- erfc(x)
see-also> erf
*/
function erfc(x) {
...
/* >>
word> besselJ
description> the [Bessel function of the first kind](https://en.wikipedia.org/wiki/Bessel_function#Bessel_functions_of_the_first_kind:_J%CE%B1)
sed> n x -- J_n(x)
*/
...
// https://en.wikipedia.org/wiki/Minkowski%27s_question-mark_function#Algorithm
/* >>
word> beseelJ
description> the [Minkowski question mark function](https://en.wikipedia.org/wiki/Minkowski%27s_question-mark_function)
sed> x -- ?(x)
*/
function minkowski(x) {
    var p = Math.floor(x);
    var q = 1, r = p + 1, s = 1, m, n, d = 1, y = p;
    if ((p < 0) ^ (r <= 0)) return undefined; // out of range [0, 1]
    while (true) {
        d /= 2;
        if (y + d === y) break; // reached max precision of float
        m = p + r;
        if ((m < 0) ^ (p < 0)) break; // overflowed
        n = q + s;
        if (n < 0) break; // overflowed
        if (x < m / n) {
            r = m;
            s = n;
        } else {
