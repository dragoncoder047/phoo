...
description>
    ...
    The second is `e`, which escapes characters in the sme manner as `\\` does in Javascript strings.
sed> -- s
...
module.macros.add('$', function string() {
    ...
    if (/^\S/.test(code)) {
        ...
        if (formatCode.startsWith('r')) {
            ...
            var res = [new RegExp(`${esc}x([0-9a-f]{2})`, 'gi'),
                       new RegExp(`${esc}u([0-9a-f]{4})`, 'gi'),
                       new RegExp(`${esc}u\{([0-9a-f]+)\}`, 'gi')];
            for (var re of res) str = str.replaceAll(re, function(match) { return String.fromCharCode(parseInt(match.groups[1], 16)); });
            const c = { '0': 0, a: 7, b: 8, t: 9, n: 10, v: 11, f: 12, r: 13, e: 27 };
            str = str.replaceAll(new RegExp(`${esc}(.)`, 'gi'), function(match) { return match[1] in c ? c[match[1]] : match[0]; });
        }
    }
...
/* >>
word> peek
description> takes the i-th item out of the array a.
sed> a i -- a[i]
*/
...
/* >>
word> ]sandbox[
description> runs the code under a `try` statement, and pushes the error it threw, or `false` if no error.
sed> c -- e
*/
...
/* >>
word> die
description> throws a `PhooError` with the message.
sed> m --
*/
