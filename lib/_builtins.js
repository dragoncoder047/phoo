...
module.macros.add('//', function lineComment() { var code = this.pop(), comment; [comment, code] = [code.slice(0, code.indexOf('\n')), code.slice(code.indexOf('\n'))]; doNothing(comment); this.push(code); });
...
module.macros.add('now!'m async function duringcomp() { var code = this.pop(); var arr = this.pop(); await this.run(arr.pop()); this.push(arr); this.push(code); });
