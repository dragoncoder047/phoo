...
i> Additional pointer to the item
see-also> roll
*/
...
module.words.add('take', function take() {
    ...
    if (a.length === 1) {
        ...
        if (fff === word('immovable')) {
            ...
            throw new IllegalOperationError('take: item is immovable');
        }
    }
    if (a.length < 1) {
        debug('take nothing');
        throw new StackUnderflowError('take: empty array');
    }
    this.push(a.pop());
});
...
module.words.add(']done[', function metaDone() {
    var oldState = this.prevState;
    this.retPop();
    if (this.state != oldState) throw 'foobar';
});
...
module.words.add("]'[", function metaLiteral() {
    var entry = this.prevState;
    if (entry === undefined) {
        debug("]'[ not inside another array");
        throw new IllegalOperationError("]'[ in toplevel array");
    }
    entry.pc++;
