...
to DEBUG do
    true if do
        nested window .console swap .debug()
    end
    drop
end
...
to __m__ do
    ...
    dup null = iff do
        ...
        $ "Version "
            ...
            $ " (" ++
            $ "https://api.github.com/repos/phoo-lang/phoo/commits" fetchJSON behead nip dup temp put .sha 7 split drop ++
            $ ` "` ++
            temp take .commit .message ' [ $ "in"ei  1 ] .split() unpack dup len 25 > if do ' [ 0 25 ] .substring() $ "..." ++ end ++
            $ `")` ++
            echo
        $ "Shell at "
            $ "https://api.github.com/repos/phoo-lang/phoo-lang.github.io/commits" fetchJSON behead nip dup temp put .sha 7 split drop ++
            $ ` ("` ++
            temp take .commit .message ' [ $ "in"ei  1 ] .split() unpack dup len 25 > if do ' [ 0 25 ] .substring() $ "..." ++ end ++
            $ `")` ++
            echo
